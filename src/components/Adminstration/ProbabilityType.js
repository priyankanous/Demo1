...
function Probability() {
  ...
  const setProbabilityTypeData = async () => {
    if (probabilityFormData.probabilityTypeName.trim() === '') {
      setIsNameEmpty(true);
    } else {
      setIsNameEmpty(false);
    }

    if (probabilityFormData.percentage.trim() === '') {
      setIsPercentageEmpty(true);
    } else {
      setIsPercentageEmpty(false);
    }

    if (!isNameEmpty && !isPercentageEmpty && isEditId !== null) {
